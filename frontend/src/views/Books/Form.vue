  ...
  <form class="space-y-6" @submit.prevent="handleSubmit">
      ...
      <div>
        <FormField name="author">
          <FormItem>
            ...
            <FormDescription
              v-if="validationErrors.author !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
      ...
      <div>
        <FormField name="isbn">
          <FormItem>
            ...
            <FormDescription
              v-if="validationErrors.isbn !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
        ...
        <FormField name="publish_date">
          <FormItem>
            ...
            <FormControl>
              <Input
                type="date"
                placeholder="Publish date of the book"
                v-model="book.publish_date"
                :disabled="isShow"
              />
            </FormControl>
            <FormDescription
              v-if="validationErrors.publish_date !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
        ...
        <FormField name="on_store">
          <FormItem>
            ...
            <FormControl>
              <Input
                type="number"
                placeholder="On store quantity"
                v-model="book.on_store"
                min="0"
                step="1"
                :disabled="isShow"
              />
            ...
            <FormDescription
              v-if="validationErrors.on_store !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
      ...
      <div>
        <FormField name="price">
          <FormItem>
            ...
            <FormDescription
              v-if="validationErrors.price !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
      ...
      <div>
        <FormField name="summary">
          <FormItem>
            ...
            <FormControl>
              <Textarea
                placeholder="Summary from the book"
                v-model="book.summary"
                :disabled="isShow"
              />
            </FormControl>
            <FormDescription
              v-if="validationErrors.summary !== 'undefined'"
              class="text-red-500 text-sm italic"
            >
              <ul>
    ...
    <div class="flex justify-end">
      ...
      <div v-if="!props.isShow">
        <RouterLink
          :to="{ name: 'books.show', params: { id: route.params.id } }"
          v-if="!props.isCreate"
        >
        ...
        <Button type="submit" v-if="!loading"> <Check class="w-4 h-4 mr-2" /> Save </Button>
      </div>
      <div v-else>
        <RouterLink :to="{ name: 'books.edit', params: { id: route.params.id } }">
          <Button type="button" variant="outline"> <Pencil class="w-4 h-4 mr-2" /> Edit </Button>
        </RouterLink>
      </div>
    </div>
  </form>
...
const fetchBook = async () => {
  ...
  loading.value = true
  axios
    .get('http://localhost:8000/api/books/' + route.params.id)
    .then((response) => {
  ...
  axios({
    ...
    .then((response) => {
      toast({
        ...
        description:
          'The book has been ' + (props.isCreate ? 'created' : 'updated') + ' successfully'
      })
